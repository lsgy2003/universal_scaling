...
outfile  = fullfile(outroot, '2^8_amp.mat');
...
parfor trial = 1:nTrials
    ...
    %vort_state = vort_init(Nt);
    freq_state   = freq_init(Nx, Nt);
    fluc_state = fluc_init_amp(Nt_settle);
    ...
    for it = 1:Nt_total
        ...
        if mod(it-1, Evo) == 0    
		 ...
		 B_init = B_now;
                 avg_B_init = sqrt(mean(abs(B_init).^2));   % same as /Nx
            end

            if have_init && it > T_settle/dt
                fluc_state = fluc_step_amp(fluc_state, B_init, avg_B_init, B_now, A_init, avg_A_init, A_now);
            end
            %
        end
    ...
    fluc_result = fluc_finalize_amp(fluc_state, fs);
